...
namespace RE::BSScript
{
	...
	public:
		...
		template <class T>
		[[nodiscard]] bool is() const //
			requires(std::same_as<T, Object> || std::same_as<T, BSTSmartPointer<Object>>)
		{
		...
		template <class T>
		[[nodiscard]] bool is() const //
			requires(std::same_as<T, Struct> || std::same_as<T, BSTSmartPointer<Struct>>)
		{
		...
		template <class T>
		[[nodiscard]] bool is() const //
			requires(std::same_as<T, Array> || std::same_as<T, BSTSmartPointer<Array>>)
		{
			return varType.IsArray();
		}

		[[nodiscard]] TypeInfo& GetType()
		{
			return varType;
		}
		...
		void SetType(const TypeInfo& a_type)
		{
			varType = a_type;
		}

		[[nodiscard]] IComplexType* GetComplexType() const
		{
			return varType.GetComplexType();
		}
	...
	template <class T>
	[[nodiscard]] std::nullptr_t get(const Variable& a_var) //
		requires(std::same_as<T, std::nullptr_t>)
	{
	...
	[[nodiscard]] stl::observer<Variable*> get(const Variable& a_var) //
		requires(std::same_as<T, Variable> || std::same_as<T, Variable*>)
	{
