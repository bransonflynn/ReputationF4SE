...
namespace RE::Scaleform::GFx::AS3
{
	...
	public:
		...
		const void* savedScope;			  // 38 - To-Do
		const Traits* originationTraits;  // 40
		void* scopeStack;				  // 48 - To-Do
		Value* prevInitialStackPos;		  // 50
		void* defXMLNamespace;			  // 58 - To-Do
	...
	public:
		...
		std::byte pad058[0x4A8 - 0x58]; // 058 - To-Do
	};
	...
	public:
		...
		{
			if (callStack.size > 0) {
				auto index = callStack.size - 1;
				return &callStack.pages[index >> 6][index & 0x3F];
			}
		...
		std::byte pad008[0xE8 - 0x18];				   // 018 - To-Do
		ArrayPagedCC<CallFrame, 6, 64, 329> callStack; // 0E8
		std::byte pad198[0x2D0 - 0x198];			   // 198 - To-Do
	};
