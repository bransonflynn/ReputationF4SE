...
namespace REX::INI
{
	...
	{
	public:
		Setting(sec_t a_section, key_t a_key, T a_default) noexcept :
...
namespace std
{
	...
	struct formatter<REX::INI::Setting<T, S>>
		: public formatter<T>
	{
		...
		constexpr auto format(const REX::INI::Setting<T, S>& a_setting, FormatContext& a_ctx) const
		{
